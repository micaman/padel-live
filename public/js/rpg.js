...
async function loadMatches() {
  setStatus("Loading matches from /api/db-matches...");
  setError("");
  try {
    const res = await fetch("/api/db-matches?limit=20&offset=0");
    if (!res.ok) throw new Error("Failed to fetch matches");
    const data = await res.json();
    matchesCache = [...(data.items || []), ...(data.extras || [])];
    populateMatchesSelect(matchesCache);
    setStatus(`Loaded ${matchesCache.length} matches`);
  } catch (e) {
