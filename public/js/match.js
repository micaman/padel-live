...
function renderDetailCards(detailData) {
  ...
  container.innerHTML = detailData.rows
    .map((row, idx) => {
      ...
      return `
        <div class="detail-card">
          <div class="detail-card__header">
            <span>${renderPlayerName(idx)}</span>
...
function derivePointEventLabel(prevSnap, currSnap, eventsForPoint = []) {
  ...
  if (latestEvent) {
    const playerLabel = escapeHtml(
      state.currentNames[latestEvent.playerIndex] ||
        `P${latestEvent.playerIndex + 1}`,
    );
