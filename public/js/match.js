...
function updateMvpBadges(snap) {
  ...
  mvpIndices.forEach((idx) => {
    const chip = document.querySelector(`.player-chip[data-player-index="${idx}"]`);
...
function normalizeEventsPayload(events, snapshotsFallback = []) {
  ...
  return (Array.isArray(snapshotsFallback) ? snapshotsFallback : []).map((snap) => ({
    id: null,
    raw: snap || {},
...
function syncAutoRefreshUi() {
  ...
  if (dom.livePill) {
    const show = state.autoRefreshEnabled && !state.isMatchFinished && Boolean(state.autoRefreshTimerId);
...
function startAutoRefresh(immediate = false) {
  ...
  if (!state.currentMatchId || state.isMatchFinished || !state.autoRefreshEnabled) return;
...
async function refreshMatchQuietly() {
  ...
  try {
    await autoLoadFromServer(state.currentMatchId, { silent: true, skipNeighbors: true });
  } catch (err) {
...
function syncAutoRefreshState(immediate = false) {
  if (!state.autoRefreshEnabled || state.isMatchFinished || !state.currentMatchId) {
    stopAutoRefresh();
  } else {
...
async function autoLoadFromServer(matchId, options = {}) {
  ...
  try {
    ...
    const snapshotsFromApi = Array.isArray(data.snapshots) ? data.snapshots : [];
...
function updateDeleteModeUi() {
  if (dom.adminActions) dom.adminActions.style.display = state.deleteMode ? "flex" : "none";
  if (dom.gameHistoryActionHeader) dom.gameHistoryActionHeader.style.display = state.deleteMode ? "" : "none";
}
...
async function deleteEventById(eventId) {
  ...
  try {
    ...
    const res = await fetch(`/api/match/${state.currentMatchId}/events/${eventId}`, {
...
async function handleDeleteMatch() {
  ...
  try {
    ...
    const res = await fetch(`/api/match/${state.currentMatchId}`, { method: "DELETE" });
